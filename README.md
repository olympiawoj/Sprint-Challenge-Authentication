...
The purpose of using sessions is to allow a server to store information about a client - we can use sessions in order to persist authentication information so that there's no need to re=-enter credentials on every new request that a client makes to a server.

1. What does bcrypt do to help us store passwords in a secure manner.

Bcrypt is a module that allows us to do three things:

- hash passwords - i.e. transforming a password into another string that is called a hashed password.
- implement salting- the bcrypt generates a salt (random string) and adds it to the password to make it longer and secure
- accumulative hashing rounds

1. What does bcrypt do to slow down attackers?

The bcrypt has an algorithm that hashes information multiple times (rounds) which slows down an attacker because they must not only need to know the hash, but they also must know the algo used and how many rounds were used to generate the hash.

1. What are the three parts of the JSON Web Token?

Three parts of a JSON web token are the header (contains algo with token type), the payload (contains claims and userid), and the signature (create a string encoding the header and payload together and signing it with a secret).
